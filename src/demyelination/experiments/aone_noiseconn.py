...
The weight of the connection from the bg noise to the exc and inh populations are different.
'''
...
def run(parameters, display=False, plot=True, save=True, load_inputs=False):
    ...
    nest.Connect(pg_aone, topology_snn.find_population('eA1').nodes, 'all_to_all', syn_spec={'weight': parameters.noise_pars.w_noise_Ectx})
    nest.Connect(pg_aone, topology_snn.find_population('iA1').nodes, 'all_to_all', syn_spec={'weight': parameters.noise_pars.w_noise_Ictx})
    ...
    precomputed = { "pearsoncoeffTen" : {
                    ...
                    "pearsoncoeffHundred" : {
                        #"MGN" : temp_mgn.pairwise_pearson_corrcoeff(nb_pairs=500, time_bin=10)[0],
                        #"TRN" : temp_trn.pairwise_pearson_corrcoeff(nb_pairs=500, time_bin=10)[0],
                        "eA1" : temp_eaone.pairwise_pearson_corrcoeff(nb_pairs=500, time_bin=100)[0],
                        "iA1" : temp_iaone.pairwise_pearson_corrcoeff(nb_pairs=500, time_bin=100)[0]
                    }
                }
