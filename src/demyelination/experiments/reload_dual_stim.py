...
def restore_network(topology_snn):
    ...
    networktime = 0 * 1000.0
    singularexp = False
    nthreads = 48

    # recurrent case
    #dump_filename = 'demyelination/data/plasticity-cadist/other'
    #networktime = 180 * 1000.0
    #singularexp = True
    #nthreads = 8
...
def run(parameters, display=False, plot=True, save=True, load_inputs=False):
    ...
    first_ng = nest.Create('poisson_generator', n=1, params={'rate': parameters.noise_pars.nuX_stim, 'start' : 2000., 'stop' : 4000.})
    ...
    second_ng = nest.Create('poisson_generator', n=1, params={'rate': parameters.noise_pars.nuX_stim, 'start' : 6000., 'stop' : 8000.})
    # connecting stimulus !!! generator to snn
