...
def run(parameters, display=False, plot=True, save=True, load_inputs=False):
    ...
    pg = nest.Create('poisson_generator', n=1, params={'rate': parameters.noise_pars.nuX_th})
    ...
    for idx in range(topology_snn.find_population('eA1').size):
        nest.Connect(pg, topology_snn.find_population('eA1').nodes[idx], 'one_to_one', syn_spec={'weight': parameters.noise_pars.w_noise_ctx})

    for idx in range(topology_snn.find_population('iA1').size):
        nest.Connect(pg, topology_snn.find_population('iA1').nodes[idx], 'one_to_one', syn_spec={'weight': parameters.noise_pars.w_noise_ctx})
