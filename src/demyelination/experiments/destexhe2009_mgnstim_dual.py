...
def run(parameters, display=False, plot=True, save=True, load_inputs=False):
    ...
    first_ng = nest.Create('poisson_generator', n=1, params={'rate': parameters.noise_pars.nuX_stim, 'start': 40000.,
                                                             'stop': 40000. + parameters.noise_pars.stim_duration})
    # connecting stimulus !!! generator to snn
    nest.Connect(first_ng, topology_snn.populations['MGN'].nodes[:40], 'all_to_all',
                 syn_spec={'weight': parameters.noise_pars.w_noise_stim})

    # second stimulus generator
    second_ng = nest.Create('poisson_generator', n=1, params={'rate': parameters.noise_pars.nuX_stim, 'start': 6000.,
                                                              'stop': 6000. + parameters.noise_pars.stim_duration})
    # connecting stimulus !!! generator to snn
    #nest.Connect(second_ng, topology_snn.populations['MGN'].nodes[41:80], 'all_to_all',
    #             syn_spec={'weight': parameters.noise_pars.w_noise_stim})
