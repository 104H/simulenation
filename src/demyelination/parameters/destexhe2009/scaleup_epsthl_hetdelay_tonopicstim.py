...
Scaling up the model led to the actiity being undesirable.
The MGN has higher activity than the TRN. The eA1 and iA1 are two close to each other.
In scaleup_gamma_intratrn.py we tried to alter the intra TRN connectivity and gamma, however it did not help.
In scaleup_epsthl_hetdelay.py, we have increased the inta Thalamic connectivity.
This experiment builds upon scaleup_epsthl_hetdelay.py

A desirable parameter combination was found. In this experiment we have explored heterogenous delays sampled from a uniform distribution from 1 to 3.
Along with heterogenous delays, there is tonotopic connectivity from the MGN to the eA1 and the MGN is stimulated.
"""
...
experiment_label = 'destexhe-scaleup-eps-thl-hetdelay'
...
################################
def build_parameters(b, nu_x, w_exc, hetdelay_thl, hetdelay_ctx):
    ...
    scale_eps_intratrn = 1

    epsilon_th = .02
    epsilon_aone = 0.0128
    
    epsilon_mgn_ctx = 0.008
    epsilon_ctx_trn = 0.008
    epsilon_ctx_mgn = 0.008
   ...
   # Excitatory synaptic elements of excitatory neurons
    ...
    '''
    if (hetdelay_ctx):
        syn_inh_aone = {'synapse_model': 'static_synapse', 'delay': nest.random.uniform(min=1., max=3.), 'weight': - gamma_aone * w_aone}
        syn_exc_aone = {'synapse_model': 'static_synapse', 'delay': nest.random.uniform(min=1., max=3.), 'weight': w_aone}
    if not(hetdelay_ctx):
    '''
