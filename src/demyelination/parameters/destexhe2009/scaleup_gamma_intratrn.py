...
Scaling up the model led to the loss of desired activity.
The MGN has higher activity than the TRN. The eA1 and iA1 are two close to each other and higher than desired.
In this experiment, we explore higher levels of gamma, along with weaker recurrent connections in the TRN.
This experiment builds upon scaleup.py, which had heterogenous delays installed (as an exploratory parameter).
"""
...
experiment_label = 'destexhe-scaleup-gamma-intratrn'
...
ParameterRange = {
    ...
    "w_exc" : [2., 3., 4., 5., 6.],
    "gamma" : np.arange(10, 16),
    "scale_eps_intratrn" : np.arange(1., 5.)
}

################################
def build_parameters(b, nu_x, w_exc, gamma, scale_eps_intratrn):
    ...
    epsilon_mgn_ctx = epsilon / thl_increase_scale
    epsilon_ctx_trn = epsilon / ctx_increase_scale
    epsilon_ctx_mgn = epsilon / ctx_increase_scale
